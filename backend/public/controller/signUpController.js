...
const signUp = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const duplicate = yield usersModel_1.default.findOne({ username });
        if (duplicate) {
            return res.status(409).json({ message: 'user with user name already exists' });
        }
        const hashedPassword = yield bcrypt.hash(password, 10);
        const token = yield jwt.sign({ username }, `${process.env.TOKEN}`);
        const newUser = yield usersModel_1.default.create({ username, password: hashedPassword });
